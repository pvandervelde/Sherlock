   ...
   public sealed class XmlReportTransformer : IReportTransformer
   {
      ...
      private static string CreateReportSectionXml(IReport report)
      {
         ...
         foreach (var reportSection in report.Sections())
         {
            ...
            {
               builder.Replace(@"${REPORT_SECTION_NAME}$", reportSection.Name);
      ...
      private static string CreateTestSectionXml(ReportSection section)
      {
         ...
         foreach (var testSection in section.Sections())
         {
            ...
            {
               builder.Replace(@"${TEST_SECTION_NAME}$", testSection.Name);
               builder.Replace(@"${TEST_SECTION_START_TIME}$", testSection.StartTime.ToString("o", CultureInfo.CurrentCulture));
               builder.Replace(@"${TEST_SECTION_END_TIME}$", testSection.EndTime.ToString("o", CultureInfo.CurrentCulture));
               builder.Replace(@"${TEST_SECTION_WAS_SUCCESSFUL}$", testSection.WasSuccessful.ToString());
         ...
         TestSection testSection)
      {
         ...
         foreach (var info in testSection.InfoMessages())
         {
            ...
            {
               builder.Replace(@"${INFO_TIME}$", info.Time.ToString("o", CultureInfo.CurrentCulture));
               builder.Replace(@"${INFO_TEXT}$", HttpUtility.HtmlEncode(info.Information));
            }
         ...
         TestSection testSection)
      {
         ...
         foreach (var info in testSection.WarningMessages())
         {
            ...
            {
               builder.Replace(@"${WARNING_TIME}$", info.Time.ToString("o", CultureInfo.CurrentCulture));
               builder.Replace(@"${WARNING_TEXT}$", HttpUtility.HtmlEncode(info.Information));
            }
         ...
         TestSection testSection)
      {
         ...
         foreach (var info in testSection.ErrorMessages())
         {
            ...
            {
               builder.Replace(@"${ERROR_TIME}$", info.Time.ToString("o", CultureInfo.CurrentCulture));
               builder.Replace(@"${ERROR_TEXT}$", HttpUtility.HtmlEncode(info.Information));
            }
      ...
      public void Transform(IReport report, Action<string, Stream> fileWriter)
      {
         ...
         { 
            ...
            builder.Replace(@"${HOST_NAME}$", report.Header.HostName);
            ...
            builder.Replace(@"${TEST_START_TIME}$", report.Header.StartTime.ToString("o", CultureInfo.CurrentCulture));
            builder.Replace(@"${TEST_END_TIME}$", report.Header.EndTime.ToString("o", CultureInfo.CurrentCulture));
         ...
         using (StreamWriter writer = new StreamWriter(internalStream, Encoding.UTF8))
         {
